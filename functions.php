...
function themesflat_setup() {
     ...
     * to change 'isak' to the name of your theme in all the template files
     */
    load_theme_textdomain( 'isak', THEMESFLAT_DIR . '/languages' );
    ...
    add_image_size( 'themesflat-blog-grid', 1126, 669, true );    
    ...
    register_nav_menus( array(
        'primary' => esc_html__( 'Primary Menu', 'isak' ),
        'one-page' => esc_html__( 'One Page Menu', 'isak' ),
    ) );
...
function themesflat_widgets_init() {
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Sidebar Blog', 'isak' ),
        'id'            => 'blog-sidebar',
        'description'   => esc_html__( 'Add widgets here to appear in your sidebar Blog Sidebar.', 'isak' ),
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Services Sidebar', 'isak' ),
        'id'            => 'themesflat-custom-sidebar-servicessidebar',
        'description'   => esc_html__( 'Add widgets here to appear in your sidebar toggler.', 'isak' ),
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
        'after_widget'  => '</div>',
        'before_title'  => '<h4 class="widget-title">',
        'after_title'   => '</h4>',
    ...
    register_sidebar( array(
        'name'          => esc_html__( 'Portfolio Sidebar', 'isak' ),
...
function themesflat_scripts() {    
    // Theme stylesheet.    
    wp_enqueue_style( 'icon-isak', THEMESFLAT_LINK . 'css/icon-isak.css' );
    ...
    wp_enqueue_style( 'tf-main', THEMESFLAT_LINK . 'css/tf-main.css' );  
...
/**
 * Register Sidebar Settings Menu
 */
function isak_register_sidebar_settings_menu() {
    add_menu_page(
        esc_html__('Sidebar Settings', 'isak'), // Page Title
        esc_html__('User Sidebar', 'isak'),     // Menu Title
        'manage_options',                       // Capability
        'isak-sidebar-settings',                // Menu Slug
        'isak_render_sidebar_settings_page',    // Callback
        'dashicons-id-alt',                     // Icon
        25                                      // Position
    );
}
add_action('admin_menu', 'isak_register_sidebar_settings_menu');

/**
 * Register Settings Group
 */
function isak_register_sidebar_settings() {
    register_setting('isak_sidebar_group', 'isak_sidebar_data');
}
add_action('admin_init', 'isak_register_sidebar_settings');

/**
 * Render Admin Page Form
 */
function isak_render_sidebar_settings_page() {
    // Get existing options
    $options = get_option('isak_sidebar_data');

    // Define Social Media List (Key => Label)
    $social_networks = [
        'facebook'  => 'Facebook',
        'x'         => 'X (Twitter)',
        'instagram' => 'Instagram',
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('User Sidebar Configuration', 'isak'); ?></h1>
        <form method="post" action="options.php">
            <?php settings_fields('isak_sidebar_group'); ?>
            <table class="form-table">
                ...
                <tr>
                    <th scope="row"><label><?php esc_html_e('Status Badge', 'isak'); ?></label></th>
                    <td>
                        <input type="text" name="isak_sidebar_data[badge]" value="<?php echo esc_attr($options['badge'] ?? 'Available for Work'); ?>" class="regular-text">
                        <p class="description"><?php esc_html_e('Default: Available for Work', 'isak'); ?></p>
                    </td>
                </tr>
    ...
    <td>
        ...
        <p class="description"><?php esc_html_e('Customize the greeting and the words that rotate with the animation effect.', 'isak'); ?></p>
                ...
                <tr>
                    ...
                    <td>
                        <textarea name="isak_sidebar_data[desc]" rows="4" class="large-text"><?php echo esc_textarea($options['desc'] ?? 'I help startups grow with smart design and no-code development, based in Cupertino, CA.'); ?></textarea>
                    </td>
                </tr>

                <tr>
                    <th colspan="2">
                        <h2 class="title"><?php esc_html_e('Social Media Links', 'isak'); ?></h2>
                        <p class="description"><?php esc_html_e('Leave empty to hide the icon.', 'isak'); ?></p>
                    </th>
                </tr>
                <?php foreach ($social_networks as $key => $label) : ?>
                    <tr>
                        <th scope="row"><label for="social_<?php echo esc_attr($key); ?>"><?php echo esc_html($label); ?></label></th>
                        <td>
                            <input type="text" id="social_<?php echo esc_attr($key); ?>" name="isak_sidebar_data[social][<?php echo esc_attr($key); ?>]" value="<?php echo esc_attr($options['social'][$key] ?? ''); ?>" class="regular-text" placeholder="https://...">
                        </td>
                    </tr>
                <?php endforeach; ?>

                <tr>
                    <th colspan="2"><h2 class="title"><?php esc_html_e('Action Buttons', 'isak'); ?></h2></th>
                </tr>
                
                <tr>
                    <th scope="row"><label><?php esc_html_e('Primary Button', 'isak'); ?></label></th>
                    <td>
                        <input type="text" name="isak_sidebar_data[btn_pri_text]" value="<?php echo esc_attr($options['btn_pri_text'] ?? 'Let’s talk'); ?>" placeholder="<?php esc_attr_e('Text', 'isak'); ?>" class="regular-text">
                        <input type="text" name="isak_sidebar_data[btn_pri_url]" value="<?php echo esc_attr($options['btn_pri_url'] ?? '#'); ?>" placeholder="<?php esc_attr_e('Link URL', 'isak'); ?>" class="regular-text">
                    </td>
                </tr>

                <tr>
                    <th scope="row"><label><?php esc_html_e('Download Button', 'isak'); ?></label></th>
                    <td>
                        <input type="text" name="isak_sidebar_data[btn_sec_text]" value="<?php echo esc_attr($options['btn_sec_text'] ?? 'Download CV'); ?>" placeholder="<?php esc_attr_e('Text', 'isak'); ?>" class="regular-text">
                        <input type="text" name="isak_sidebar_data[btn_sec_url]" value="<?php echo esc_attr($options['btn_sec_url'] ?? '#'); ?>" placeholder="<?php esc_attr_e('Link URL', 'isak'); ?>" class="regular-text">
                    </td>
                </tr>

            </table>
            <?php submit_button(esc_html__('Save Settings', 'isak')); ?>
        </form>
    </div>
    <?php
}

function isak_register_menus() {
    register_nav_menus([
        'mobile_menu' => __('Mobile Menu', 'isak'),
    ]);
}
add_action('after_setup_theme', 'isak_register_menus');

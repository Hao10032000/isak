...
function themesflat_customize_register( $wp_customize ) {
   ...
   //___FOOTER___//
    ...
    //___LAYOUT___//
    $wp_customize->get_section( 'background_image' )->title = esc_html__('Layout Style', 'isak');
...
function themesflat_sanitize_bg_size( $input ) {
    $valid = array(
        'cover'     => esc_html__('Cover', 'isak'),
        'contain'   => esc_html__('Contain', 'isak'),
    );
...
function themesflat_sanitize_blog( $input ) {
    $valid = array(
        ...
        'fullwidth'  => esc_html__( 'Full width (no sidebar)', 'isak' )
...
function themesflat_sanitize_pagination ( $input ) {
    $valid = array(
        'pager' => esc_html__('Pager', 'isak'),
        'numeric' => esc_html__('Numeric', 'isak'),
        'page_numeric' => esc_html__('Pager & Numeric', 'isak')                
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_related_post ( $input ) {
    $valid = array(
        'simple_list' => esc_html__('Simple List', 'isak'),
        'grid' => esc_html__('Grid', 'isak')        
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_fw( $input ) {
    $valid = array(
        '0' => esc_html__('footer_default', 'isak'),
        '1' => esc_html__('One', 'isak'),
        '2' => esc_html__('Two', 'isak'),
        '3' => esc_html__('Three', 'isak'),
        '4' => esc_html__('Four', 'isak')
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_related_portfolio( $input ) {
    $valid = array(
        'grid'                 => esc_html__( 'Grid', 'isak' ),
        'grid_masonry'         => esc_html__( 'Grid Masonry', 'isak' ),
        'grid_nomargin'        => esc_html__( 'Grid Masonry No Margin', 'isak' ),
        'carosuel'             => esc_html__( 'Carosuel', 'isak' ),
        'carosuel_nomargin'    => esc_html__( 'Carosuel No Margin', 'isak' )       
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_pagination( $input ) {
    $valid = array(
        'page_numeric'         => esc_html__( 'Pager & Numeric', 'isak' ),
        'load_more'         => esc_html__( 'Load More', 'isak' )     
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_order( $input ) {
    $valid = array(
        'date'          => esc_html__( 'Date', 'isak' ),
        'id'            => esc_html__( 'Id', 'isak' ),
        'author'        => esc_html__( 'Author', 'isak' ),
        'title'         => esc_html__( 'Title', 'isak' ),
        'modified'      => esc_html__( 'Modified', 'isak' ),
        'comment_count' => esc_html__( 'Comment Count', 'isak' ),
        'menu_order'    => esc_html__( 'Menu Order', 'isak' )     
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_order_direction( $input ) {
    $valid = array(
        'DESC' => esc_html__( 'Descending', 'isak' ),
        'ASC'  => esc_html__( 'Assending', 'isak' )       
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_grid_portfolio( $input ) {
    $valid = array(
        'portfolio-two-columns'     => esc_html__( '2 Columns', 'isak' ),
        'portfolio-three-columns'     => esc_html__( '3 Columns', 'isak' ),
        'portfolio-four-columns'     => esc_html__( '4 Columns', 'isak' ),
        'portfolio-five-columns'     => esc_html__( '5 Columns', 'isak' )
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_grid_portfolio_related( $input ) {
    $valid = array(
        'portfolio-one-columns'     => esc_html__( '1 Columns', 'isak' ),
        'portfolio-two-columns'     => esc_html__( '2 Columns', 'isak' ),
        'portfolio-three-columns'     => esc_html__( '3 Columns', 'isak' ),
        'portfolio-four-columns'     => esc_html__( '4 Columns', 'isak' ),
        'portfolio-five-columns'     => esc_html__( '5 Columns', 'isak' )
...
function themesflat_sanitize_grid_post_related( $input ) {
    $valid = array(        
        2    => esc_html__( '2 Columns', 'isak' ),
        3    => esc_html__( '3 Columns', 'isak' ),
        4    => esc_html__( '4 Columns', 'isak' ), 
        5    => esc_html__( '5 Columns', 'isak' ),       
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_layout_product( $input ) {
    $valid = array(        
        'fullwidth'         => esc_html__( 'No Sidebar', 'isak' ),
        'sidebar-right'           => esc_html__( 'Sidebar Right', 'isak' ),
        'sidebar-left'         => esc_html__( 'Sidebar Left', 'isak' )
    );
    if ( array_key_exists( $input, $valid ) ) {

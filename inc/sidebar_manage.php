...
class sidebar_manager {
	...
	public static function add_sidebar() {
		...
		if ( ! $name || $name == 'null' ) {
			die( "alert('".esc_html__( 'Please input sidebar name and try again.', 'isak' )."')" );
		}
		$id = sidebar_manager::name_to_class( $name );
		if ( isset( $sidebars[ $id ] ) ) {
			die( "alert('".esc_html__( 'Sidebar already exists, please use a different name.', 'isak' )."')" );
		}
	...
	public static function admin_menu() {
		add_theme_page( esc_html__( 'Sidebars Manager', 'isak' ), esc_html__( 'Sidebars Manager', 'isak' ), 'manage_options', 'multiple_sidebars', array( 'sidebar_manager', 'admin_page' ) );
	}
	...
	public static function admin_page() {
		...
		<script>
			function remove_sidebar_name(elem,name){
				answer = confirm("<?php echo esc_html__( 'Are you sure you want to remove', 'isak' ); ?> " + name + "?\n<?php echo esc_html__( 'This will remove any widgets you have assigned to this sidebar.', 'isak' ); ?>");
				if(answer){
			...
			function add_sidebar_name(){
				var sidebar_name = prompt("<?php echo esc_html__( 'Sidebar Name:', 'isak' ); ?>","");
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php echo esc_html__( 'Sidebar Generator', 'isak' ); ?></h2>
			<p>
				<?php echo esc_html__( 'The sidebar name is for your use only. It will not be visible to any of your visitors. A CSS class is assigned to each of your sidebar, use this styling to customize the sidebars.', 'isak' ); ?>
			</p>
			<br />
			<div class="add_sidebar">
				<a href="<?php echo esc_js( 'javascript:void(0)' ); ?>" onclick="return add_sidebar_name()" title="<?php echo esc_html( 'Add a sidebar', 'isak' ); ?>" class="button-primary"><?php echo esc_html__( '+ Add Sidebar', 'isak' ); ?></a>
			</div>
			<br />
			<table class="widefat page" id="themesflat_table" >
				<tr>
					<th><?php echo esc_html__( 'Name', 'isak' ); ?></th>
					<th><?php echo esc_html__( 'CSS class', 'isak' ); ?></th>
					<th><?php echo esc_html__( 'Remove', 'isak' ); ?></th>
				</tr>
				<?php
				$sidebars = sidebar_manager::get_sidebars();
				if ( is_array( $sidebars ) && ! empty( $sidebars ) ) {
					...
					foreach ( $sidebars as $sidebar ) {
						...
						<tr class="<?php echo esc_attr( $alt ); ?>">
							...
							<td><a href="<?php echo esc_js( 'javascript:void(0)' ); ?>" onclick="return remove_sidebar_name(this,'<?php echo esc_attr( $sidebar ); ?>');" title="<?php echo esc_html( 'Remove this sidebar', 'isak' ); ?>"><?php echo esc_html__( 'remove', 'isak' ); ?></a></td>
						</tr>
						<?php
						$cnt++;
					}
					...
					<tr id="themesflat_noexist">
						<td colspan="3"><?php echo esc_html__( 'No Sidebars defined', 'isak' ); ?></td>
					</tr>
					<?php
				}
				?>
	...
	public static function edit_form() {
		...
		<div id='themesflat-sortables' class='meta-box-sortables'>
			<div id="themesflat_box" class="postbox " >
				<div class="handlediv" title="Click to toggle"><br /></div><h3 class='hndle'><span><?php echo esc_html__( 'Sidebars', 'isak' ); ?></span></h3>
				<div class="inside">
					<div class="themesflat_container">
						<input name="themesflat_edit" type="hidden" value="themesflat_edit" />

						<p><?php echo esc_html__( 'Please select the sidebar you would like to display.', 'isak' ); ?> <strong><?php echo esc_html__( 'Note:', 'isak' ); ?></strong> <?php echo esc_html__( 'You must first create the sidebar under', 'isak' ); ?> <strong><?php echo esc_html__( 'Appearance > Sidebars', 'isak' ); ?></strong>.
						</p>
						<ul>
							<?php

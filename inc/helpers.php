...
function themesflat_shortcode_default_id(){
    return array(
                ...
                'group' => esc_html__( 'Design Options', 'isak' ),
...
function themesflat_predefined_header_styles() {
    ...
    if ( empty( $styles ) ) {
        $styles = apply_filters( 'themesflat/header_styles', array(
            'header-v1' => esc_html__( 'Classic', 'isak' ),
            'header-v2' => esc_html__( 'Header style 02', 'isak' ),
            'header-v4' => esc_html__( 'Modern', 'isak' ),
        ) );
    }
...
function themesflat_available_social_icons() {
    $icons = apply_filters( 'themesflat_available_icons', array(
        'twitter'        => array( 'iclass' => 'icon-isak-x', 'title' => 'Twitter','share_link' => THEMESFLAT_PROTOCOL . '://twitter.com/intent/tweet?url=' ),
        'facebook'       => array( 'iclass' => 'icon-isak-facebook', 'title' => 'Facebook','share_link'=> THEMESFLAT_PROTOCOL . '://www.facebook.com/sharer/sharer.php?u=' ),
        'chat'  => array( 'iclass' => 'icon-isak-chat', 'title' => 'Chat','share_link'=> THEMESFLAT_PROTOCOL . '://www.chat.com/share?url=' ),
        'pinterest'      => array( 'iclass' => 'icon-isak-pinterest', 'title' => 'Pinterest','share_link' => THEMESFLAT_PROTOCOL . '://pinterest.com/pin/create/bookmarklet/?url=' ),
        'instagram'      => array( 'iclass' => 'icon-isak-instagram', 'title' => 'Instagram','share_link' => 'https://www.instagram.com/?url=' ),
        'youtube'        => array( 'iclass' => 'icon-isak-youtube', 'title' => 'Youtube','share_link' =>'' ),
        'vimeo'          => array( 'iclass' => 'icon-isak-vimeo', 'title' => 'Vimeo','share_link' =>'' ),
        'behance'        => array( 'iclass' => 'icon-isak-behance', 'title' => 'Behance','share_link' =>'' ),
        'bitcoin'        => array( 'iclass' => 'icon-isak-bitcoin', 'title' => 'Bitcoin','share_link' =>'' ),
        'digg'           => array( 'iclass' => 'icon-isak-digg', 'title' => 'Digg','share_link' =>'http://digg.com/submit?url=' ),
        'skype'          => array( 'iclass' => 'icon-isak-skype', 'title' => 'Skype','share_link' => THEMESFLAT_PROTOCOL . '://web.skype.com/share?url='),
        'slack'          => array( 'iclass' => 'icon-isak-slack', 'title' => 'Slack','share_link' => ''),
        'spotify'        => array( 'iclass' => 'icon-isak-spotify', 'title' => 'Spotify','share_link' => ''),
        'stack-overflow' => array( 'iclass' => 'icon-isak-stackoverflow', 'title' => 'Stack Overflow','share_link' => ''),
        'steam'          => array( 'iclass' => 'icon-isak-steam', 'title' => 'Steam','share_link' => ''),
        'dribble'          => array( 'iclass' => 'icon-isak-dribbble', 'title' => 'Dribble','share_link' => ''),
        'linkedin'          => array( 'iclass' => 'icon-isak-linkedin', 'title' => 'Linkedin','share_link' => ''),    
    ) );
...
function themesflat_menu_fallback() {
    ...
    <li>
    <a class="menu-fallback" href="' . esc_url(admin_url('nav-menus.php')) . '">' . esc_html__( 'Create a Menu', 'isak' ) . '</a></li></ul>';
}
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
    ...
    function themesflat_wp_title( $title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            $title .= " $sep " . sprintf( esc_html__( 'Page %s', 'isak' ), max( $paged, $page ) );
        }
            ...
            <ul class="unstyled">
                <li class="current">                    
                    <a href="?lang=en" class="lang_sel_sel">
                        <span class="languages-before-icon icon-isak-web"></span><?php echo esc_html__("English",'isak');?><i class="icon-isak-arrow-down" aria-hidden="true"></i>
                    </a>
                    <ul class="unstyled-child">
                       <li class="icl-en">
                            <a href="?lang=en" class="lang_sel_sel">
                             <?php echo esc_html__("English",'isak');?>
                            </a>
                       </li>
                       <li class="icl-fr fr">
                            <a href="?lang=fr" class="lang_sel_other">
                             <?php echo esc_html__("French",'isak');?>
                            </a>
                       </li>
                       <li class="icl-ge ge">
                            <a href="?lang=it" class="lang_sel_other">
                                <?php echo esc_html__("German",'isak');?>
                            </a>
                       </li>
                    </ul>
                </li>
...
function themesflat_custom_search_form( $form ) {
    ...
    <label>
        <span class="screen-reader-text">' . esc_html__( 'Search for:' , 'isak' ) . '</span>
        <input type="search" value="' . get_search_query() . '" name="s" class="s" placeholder="' . esc_html__( "Search…", "isak" ) . '"/>
    </label>
    <button type="submit" class="search-submit"><i class="icon-isak-search"></i></button>    
    </form>';
...
function themesflat_social_single() {
    if( themesflat_get_opt('show_social_share') == 1 ):
        ...
        ?>
        <div class="social-share-article"><h5><?php echo esc_html__( 'Share', 'isak' ); ?></h5>        
...
function themesflat_get_page_titles() {
    ...
    if ( ! is_archive() ) {       
        ...
        elseif ( is_page() ) {
            ...
            $title = esc_html__( '404', 'isak' );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'isak' ), get_search_query() );
        } else {
            $title = get_post_meta( get_the_ID(), 'custom_title', true );
            if ( ! $title ) {
                $title = get_the_title();
        ...
        if ( is_author() ) {
            the_post();
            $title = sprintf( esc_html__( 'Author: %s', 'isak' ), get_the_author() );
            rewind_posts();
        } elseif ( is_day() ) {
            $title = sprintf( esc_html__( 'Daily: %s', 'isak' ), get_the_date() );
        } elseif ( is_month() ) {
            $title = sprintf( esc_html__( 'Monthly: %s', 'isak' ), get_the_date( 'F Y' ) );
        } elseif ( is_year() ) {
            $title = sprintf( esc_html__( 'Yearly: %s', 'isak' ), get_the_date( 'Y' ) );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'isak' ), get_search_query() );
            ...
            $title = esc_html__('Book Appointment','isak');           
...
class Isak_Mobile_Menu_Walker extends Walker_Nav_Menu {

    function start_el(&$output, $item, $depth = 0, $args = null, $id = 0) {
    error_log(print_r($item, true));


        $icon_class = get_post_meta($item->ID, '_menu_icon', true);
        ...
        $output .= '<a href="' . esc_url($url) . '" class="item-link scroll-link">';

        if (!empty($icon_class)) {
            $output .= '<i class="icon ' . esc_attr($icon_class) . '"></i>';
        }

        $output .= '<p class="tool-tip text-caption">' . esc_html($title) . '</p>';
...
// Add icon field to menu
function isak_add_icon_field($item_id, $item, $depth, $args) {
    $icon = get_post_meta($item_id, '_menu_icon', true);
    ?>
    <p class="description description-wide">
        <label>
            Menu Icon Class<br>
            <input type="text" name='menu_icon[<?php echo esc_attr( $item_id ); ?>]' 
                value="<?php echo esc_attr($icon); ?>" 
                placeholder="icon-isak-home" style="width:100%;" />
        </label>
    </p>
    <?php
}
add_action('wp_nav_menu_item_custom_fields', 'isak_add_icon_field', 10, 4);

// Save icon
function isak_save_icon_field($menu_id, $menu_item_db_id) {
    if (isset($_POST['menu_icon'][$menu_item_db_id])) {
        update_post_meta(
            $menu_item_db_id,
            '_menu_icon',
            sanitize_text_field($_POST['menu_icon'][$menu_item_db_id])
        );
    }
}
add_action('wp_update_nav_menu_item', 'isak_save_icon_field', 10, 2);
